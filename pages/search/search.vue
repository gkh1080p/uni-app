...
import {getSearchWall} from '@/api/search/index.js'
...
const onSearch = ()=>{
	uni.showLoading()
	historySearch.value=Array.from(new Set([queryParams.value.keyword,...historySearch.value])).slice(0,10)
...
const searchData=async ()=>{
	try{
		let res=await getSearchWall(queryParams.value.keyword,queryParams.value.pageNum,queryParams.value.pageSize)
